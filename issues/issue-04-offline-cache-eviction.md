...
- `app/public/sw.js:18` — `CARD_CACHE` name is versioned (`henbridge-cards-v1`) only for wholesale cache-busting on a deploy (via `activate`'s cleanup of non-matching cache names), not for bounding growth within a version.
