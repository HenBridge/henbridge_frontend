...
## Constraints
- The on-chain contract interface changes only if unavoidable, and any such change must be called out explicitly as a cross-repo, breaking change to `henbridge-contracts`'s `Attestation` struct — prefer a solution that keeps `record_hash: BytesN<32>` and the existing `attest`/`get_attestation` signatures untouched if at all possible.
...
- The `henbridge-contracts` Rust implementation itself, beyond documenting any required interface change.
- Building the actual CHW re-attestation UI/tool (`henbridge-verifier`) — you only need to wire the patient-facing "request re-verification" trigger to *something* real and testable (e.g., a queued request record), not a full CHW-side experience.
